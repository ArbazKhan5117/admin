...
class EditLevel extends Component {
    ...
    fetchSubjects() {
        ...
        axios.post('http://localhost/fyp-backend/signup/editLevels.php', fd, headers
        ).then(res => {
    ...
    submitNewSubject(event) {
        ...
        axios.post('http://localhost/fyp-backend/signup/addNewLevel.php', fd, headers
        ).then(res => {
            if(res.data.valid==='yes'){
                alert(`${this.state.newSubject} is successfully added in Level Choice`);
    ...
    deleteSubject(){
        ...
        axios.post('http://localhost/fyp-backend/signup/deleteExistingLevel.php', fd, headers
        ).then(res => {
        ...
        return (
            <div className="editSubject-class">
                ...
                <form onSubmit={this.submitNewSubject}>
                    <input type="text" required value={this.state.newSubject} onChange={this.subjectHandler} placeholder="Enter New Level" className="login-input" />
